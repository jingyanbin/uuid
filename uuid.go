...
func (m *Uint64Generator) getTimeIndex() (int64, int64) {
	...
	for nowTime < m.lastTime {
		...
		if backTime < interval { //时间回拨超过1秒 一般
			log.WarnF("uint64 uuid getTimeIndex back in time: %s", datetime.UnixToYmdHMS(m.lastTime/1000, timezone.Local()))
		}
		time.Sleep(interval)
		nowTime = datetime.UnixMs()
	}
...
func (m *Uint64Generator) getTimeIndexNow() (int64, int64) {
	...
	for nowTime < m.lastTime {
		interval := time.Duration(m.lastTime-nowTime) * time.Millisecond
		if backTime < interval { //时间回拨超过1秒 一般
			log.WarnF("uint64 uuid getTimeIndexNow back in time: %s", datetime.UnixToYmdHMS(m.lastTime/1000, timezone.Local()))
		}
		time.Sleep(interval)
		nowTime = datetime.UnixMs()
	}
...
func (m *Uint64Generator) UUID() uint64 {
	...
	if value > m.timeMax || value < m.timeMin {
		panic(basal.NewError("uuid time out of range: %s~%s, %s", m.dateTimeMin, m.dateTimeMax, datetime.UnixToYmdHMS(ms/1000, timezone.Local())))
	}
...
//workerId: 服务进程唯一ID 范围: 0~（1<<workerIdBits-1）
...
func NewUUIDGenerator(workerId, workerIdBits, epoch int, latest bool, unsigned bool) *Uint64Generator {
	if workerIdBits < 0 || workerIdBits > 21 {
		panic(basal.NewError("new uint64 generator workerIdBits out of range: 1~21, %d", workerIdBits))
	}
	workerIdMax := 1<<workerIdBits - 1
	if workerId > workerIdMax || workerId < 0 {
		panic(basal.NewError("new uint64 generator workerId out of range: 0~%d, %d", workerIdMax, workerId))
	}
	if epoch < 1 {
		epoch = defaultEpoch
	}
	...
	if ms-int64(epoch) < timeMin {
		panic(basal.NewError("new uint64 generator now time less than time min value: %d", timeMin))
	}

	if ms-int64(epoch) > timeMax {
		panic(basal.NewError("new uint64 generator now time more than time max value: %d", timeMax))
	}
	...
	generator.dateTimeMax = datetime.UnixToYmdHMS(generator.epoch+generator.timeMax, timezone.Local())
	generator.dateTimeMin = datetime.UnixToYmdHMS(generator.epoch+generator.timeMin, timezone.Local())
...
var hexDateTimeMin = datetime.UnixToYmdHMS(defaultEpoch+hexTimeMin, timezone.Local())
...
func (m *HexGenerator) getTimeIndexNow() (int64, int64) {
	...
	for nowTime < m.lastTime {
		interval := time.Duration(m.lastTime-nowTime) * time.Millisecond
		if backTime < interval { //时间回拨超过1秒 一般
			log.WarnF("uint64 uuid hex generator getTimeIndexNow back in time: %s", datetime.UnixToYmdHMS(m.lastTime/1000, timezone.Local()))
		}
		time.Sleep(interval)
		nowTime = datetime.UnixMs()
	}
...
func (m *HexGenerator) UUID() string {
	...
	if timeValue > hexTimeMax || timeValue < hexTimeMin {
		panic(basal.NewError("uuid time out of range: %s~%s, %s", hexDateTimeMin, hexDateTimeMax, datetime.UnixToYmdHMS(ms/1000, timezone.Local())))
	}
	data := make([]byte, 12)
...
func (m *HexGenerator) DeUUID(uuid string) (ms int64, index int, workerId uint16, err error) {
	if len(uuid) != 24 {
		err = basal.NewError("decode uuid length no is 26")
		return
	}
	var data []byte
	...
	bcc := m.bcc(data, 0, 11)
	if bcc != data[11] {
		err = basal.NewError("decode uuid bcc check failed")
		return
	}
...
func (m *HexGenerator) UUIDExtra(extra uint16) string {
	...
	timeValue := ms - defaultEpoch
	if timeValue > hexTimeMax || timeValue < hexTimeMin {
		panic(basal.NewError("uuid extra time out of range: %s~%s, %s", hexDateTimeMin, hexDateTimeMax, datetime.UnixToYmdHMS(ms/1000, timezone.Local())))
	}
	data := make([]byte, 14)
	head := uint64((timeValue << hexTimeShift) | index)
	binary.BigEndian.PutUint32(data[0:], uint32(head))
...
func (m *HexGenerator) DeUUIDExtra(uuid string) (ms int64, index int, workerId uint16, extra uint16, err error) {
	if len(uuid) != 28 {
		err = basal.NewError("decode uuid extra length no is 26")
		return
	}
	...
	bcc := m.bcc(data, 0, 13)
	if bcc != data[13] {
		err = basal.NewError("decode uuid extra bcc check failed")
		return
	}
	h1 := int64(binary.BigEndian.Uint32(data[0:]))
	h2 := int64(binary.BigEndian.Uint16(data[4:]))
